...
class TfliteSoundClassifier:
    ...
    def predict_spectrogram_class(self, image_path, prediction_type = 'label'):
        ...
        if prediction_type not in ('label', 'idx'):
            raise ValueError('prediction_type must be "label" or "idx"')
    ...
    def predict(self, audio_file, prediction_type = 'label'):
        ...
        return self.predict_spectrogram_class(spectrogram_path, prediction_type)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_file', 
        default= None, 
        help = "Path to .tflite saved model file.", 
        required = True)
    parser.add_argument('--model_labels_file', 
        default= None, 
        help = "Path to .txt saved labels file for model.", 
        required = True)
    parser.add_argument('--audio_file', 
        default= None, 
        help = "Path to .wav saved audio file to classify.", 
        required = True)
    args = parser.parse_args()

    MODEL_FILE = args.model_file
    LABELS_FILE = args.model_labels_file
    AUDIO_FILE = args.audio_file

    audioClassifier = TfliteSoundClassifier(MODEL_FILE, LABELS_FILE)

    print("\n", audioClassifier.predict(AUDIO_FILE))
